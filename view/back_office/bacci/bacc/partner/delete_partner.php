...
if (!isset($_GET['id']) || empty($_GET['id'])) {
    header('Location: index.php?error=' . urlencode('Partner ID is required for deletion'));
    exit;
}
...
if ($partnerController->deletePartner($partnerId)) {
    header('Location: index.php?success=' . urlencode('Partner "' . htmlspecialchars($partner['name']) . '" has been deleted successfully'));
} else {
    header('Location: index.php?error=' . urlencode('Failed to delete partner'));
}
exit;
?>
